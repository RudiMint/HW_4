...
def save_data_from_form(data):
    ...
    try:
        parse_dict = {datetime.now().strftime("%m/%d/%Y, %H:%M:%S"): {key: value for key, value in
                                                                      [el.split("=") for el in parse_data.split("&")]}}
        with open("storage/db.json", "w", encoding="utf-8") as file:
...
def run_socket_server(host, port):
    ...
    logging.info(f"Socket server started with {host}:{port}")
    try:
        while True:
            msg, address = server_socket.recvfrom(BUFFER_SIZE)
            logging.info(f"Socked received {msg} from {address}")
...
def run_http_server(host, port):
    ...
    logging.info(f"Http server started with {host}:{port}")
    try:
